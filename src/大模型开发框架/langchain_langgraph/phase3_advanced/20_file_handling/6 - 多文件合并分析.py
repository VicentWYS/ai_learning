...
def multi_file_analysis(txt_path: str, csv_path: str, json_path: str):
    ...
    with open(txt_path, "r", encoding="utf-8") as f:
        documents.append(
            Document(
                page_content=f.read(),
                metadata={"source": txt_path, "type": "txt data"},
            )
        )
    ...
    with open(csv_path, "r", encoding="utf-8") as f:
        documents.append(
            Document(
                page_content=f.read(),
                metadata={"source": csv_path, "type": "csv data"},
            )
        )
    ...
    with open(json_path, "r", encoding="utf-8") as f:
        documents.append(
            Document(
                page_content=f.read(),
                metadata={"source": json_path, "type": "json data"},
            )
        )
    ...
    messages = [
        SystemMessage(
            content="你是一个综合分析专家。请分析以下多个文件的内容，找出它们之间的联系，并给出综合见解。用中文回答。"
        ),
...
if __name__ == "__main__":
    ...
    # 运行示例
    documents = multi_file_analysis(
        txt_path=txt_path, csv_path=csv_path, json_path=json_path
    )
